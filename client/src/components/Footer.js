...
function Footer() {
  ...
  return (
    <footer style={{ borderTop: '1px solid var(--border-color, #000)', padding: '16px', marginTop: '32px', textAlign: 'center', fontSize: '0.9em', color: 'var(--text-secondary, #aaa)', background: 'var(--background-secondary, transparent)' }}>
      ...
      <div style={{ marginTop: '8px' }}>
        ...
        {!user && (
          <Link to="/login?mode=admin" style={{ margin: '0 12px', color: '#ff4444', fontWeight: 'bold' }}>
            Admin Login
          </Link>
        )}
        {user && (
